...
import { deleteCardForCrud, deleteCardSetForCrud, getCardDropRateTable, getCardsCL, cardSetClaim, getCardSetsForCrud, getCardsForCrud, postCardDropRateTable, postCardForCrud, postCardSetsForCrud, getCardTrade, postChestForFront, RewardChest } from './slot.services/card.service'
...
export async function spinGet(req: Request, res: Response): Promise<any>{
  console.log('req.user.isDev', req.user.isDev)
...
export async function cardForFrontPost(req: Request, res: Response): Promise<void> {
    const resp = await postCardForCrud(req.fields, req.files)
    res.status(200).json(resp)
}
...
export async function cardDropRateTableGet(req: Request, res: Response): Promise<void> {
  const table = await getCardDropRateTable()
  res.status(200).send(table)
}
export async function cardDropRateTablePost(req: Request, res: Response): Promise<void> {
  await postCardDropRateTable(req.body)
  res.status(200).send({'status': 'ok'})
}
...
export async function cardSetClaimGet(req: Request, res: Response): Promise<void> {
  await cardSetClaim(Number(req.query.setId), req.user.id)
  res.status(200).send({'status': 'ok'})
}
