...
export async function getAll(): Promise<User> {
  const [users]: Array<User> = await queryMeta('select * from user limit 2')
  return users
}
const getGame = async (name) => {
  const [gameRows] = await queryMeta(`SELECT * FROM game WHERE UPPER(name) = '${name.toUpperCase()}'`)
...
export const getUserById = async (id: number): Promise<User> => {
  const userSelect = `
        select *
          from user
        where id = "${id}"`
  const [rows] = await queryMeta(userSelect)
...
export const auth = async (user: User): Promise<User> => {
  ...
  `
  return await queryOneMeta(select) as User
}
