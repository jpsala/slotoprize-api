...
export const getLocalization = async (item: string, userId?: number, defaultText ?: string): Promise<string> => {
  const defaultLanguage = (userId && userId !== -1) ? await getLanguage(userId) : await getDefaultLanguage()
  try {
      ...
      if (!row)
        ...
        else {throw createHttpError(StatusCodes.BAD_REQUEST, `There is no localization for "${item} in ${defaultLanguage.languageCode}`)}
...
export const updateLocalizationJSON = async (languageCode: string, environment: string): Promise<string> => {
  if(!languageCode || !environment) throw createHttpError(StatusCodes.BAD_REQUEST, 'languageCode and environment are required parameters')
  ...
  if(!localizationJsonUrl.includes('<languageCode>')) throw createHttpError(StatusCodes.BAD_REQUEST, 'Update URL does not contains <languageCode>')
  if(!localizationJsonUrl.includes('<environment>')) throw createHttpError(StatusCodes.BAD_REQUEST, 'Update URL does not contains <languageCode>')
  ...
  if(json && json.data && json.data.msg) throw createHttpError(StatusCodes.BAD_REQUEST, json.data.msg)
...
export const getLocalizationJSON = async (languageCode: string): Promise<any> => {
  ...
  `))
  if(!json) throw createHttpError(StatusCodes.BAD_REQUEST, 'There is no localization for ' + languageCode)
  // eslint-disable-next-line @typescript-eslint/no-unsafe-return
...
export const postSettingsForLocalization = async (settings: any): Promise<any> => {
  ...
  if(!localizationJsonUrl.includes('<languageCode>')) throw createHttpError(StatusCodes.BAD_REQUEST, 'localizationJsonUrl bad formed, please include <languageCode> where the parameter while be inserted')
